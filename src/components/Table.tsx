...
class Table extends React.Component<ITableProps, ITableState> {
  ...
  filterEvents(events: Event[]) {
    return events
      .filter((e) =>
        this.state.hidePastEvents ? e.fromDate.isAfter(moment()) : true
      )
      .filter((e) =>
        this.state.selectedState ? e.state === this.state.selectedState : true
      )
      .filter((e) =>
        this.state.tagFilter
          ? e.tags.some(
              (t) =>
                t.toLowerCase().indexOf(this.state.tagFilter.toLowerCase()) >= 0
            )
          : true
      );
  }
    ...
    let rows = events.map((event, i) => {
      return (
        <tr key={i}>
          ...
          <td>{event.state}</td>
          <td>
            {event.fromDate.format("dddd, MMMM Do YYYY")}{" "}
            {event.toDate.isSame(event.fromDate)
              ? ""
              : `to ${event.toDate.format("dddd, MMMM Do YYYY")}`}
          </td>
            ...
            <th>
              ...
              <div>
                <label className="checkbox">
                  <input
                    type="checkbox"
                    onChange={(e) => this.togglePastEvents(e.target.checked)}
                    checked={this.state.hidePastEvents}
                  />{" "}
                  Hide past events
                </label>
              </div>
            ...
            <th>
              <div className="control has-icons-right">
                <input
                  ...
                  placeholder="Tags..."
                  type="search"
                  onChange={(e) => this.filterByTags(e.target.value)}
                />
